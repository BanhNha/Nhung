...
void OpenKeyPad(void)
{
  GPIO_SetMode(PA, BIT0, GPIO_PMD_QUASI); // SW SET connect sever(PA0)
	//
  GPIO_SetMode(PA, BIT4, GPIO_PMD_QUASI); // SW Xuong(PA4)
  GPIO_SetMode(PA, BIT5, GPIO_PMD_QUASI); // SW Len(PA5)
  GPIO_SetMode(PA, BIT11, GPIO_PMD_QUASI);// SW Qua Trai (PA11)
  GPIO_SetMode(PA, BIT15, GPIO_PMD_QUASI);// SW Qua Phai(PA15)
  //
  ...
  //
	...
	PA1=0;
	GPIO_SetMode(PA, BIT2, GPIO_PMD_OUTPUT);// LeD Do
	PA2=1;
}
